...
# Mengatur path agar file CSV tetap terbaca
...
with st.sidebar:
    ...
    st.caption(
        "Filter tanggal ini akan memengaruhi ringkasan performa, tren bulanan, "
        "kategori produk, dan segmentasi pelanggan."
    )

    st.write(
        f"Periode data tersedia: {min_date.date()} sampai {max_date.date()}"
    )
...
# Jika tidak ada data pada rentang tanggal yang dipilih
if filtered_data.empty:
    st.warning("Tidak ada data pada rentang tanggal yang dipilih.")
    st.stop()


# Menghapus duplikasi order agar payment_value tidak terhitung berulang
...
# Membuat data tren bulanan tanpa resample agar stabil di berbagai versi pandas
monthly_filtered_df = order_filtered_df.copy()

monthly_filtered_df["month"] = (
    monthly_filtered_df["order_purchase_timestamp"]
    .dt.to_period("M")
    .astype(str)
)

monthly_filtered_df = monthly_filtered_df.groupby("month").agg({
    "order_id": "nunique",
    "payment_value": "sum"
}).reset_index()
...
monthly_filtered_df = monthly_filtered_df.sort_values("month")
...
if len(rfm_filtered_df) >= 4:
    ...
    rfm_filtered_df["customer_segment"] = (
        rfm_filtered_df["total_score"].apply(customer_segment)
    )
